...
- - Host functions, such a standard main() are processed by the systems compiler, like as gcc
...
### Caches
On GPU's of compute capabiity 2.0 and above there is an l1 cache per multiprocessor.
Global and local memory use these.
- l1 is very fast
- - l1 and shared memory are actually the same bytes and can be configured for 48k shared, 16k l1. Or 16k shared, 48k l1
- All global memory accesses go through l2 cache, including those by the CPU
- - Can switch caching on/off via compiler option
- - Texture and constant memory have their own dedicated caches
